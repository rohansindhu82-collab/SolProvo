...
export default function DemoFactoryPage() {
  const [budget,setBudget]=useState(60),[submitted,setSubmitted]=useState(false),[prospect,setProspect]=useState<WorkspaceProspect|null>(null),[customer,setCustomer]=useState("Demo Customer"),[visitAt,setVisitAt]=useState(""),[leadId,setLeadId]=useState("");
  ...
  function requestVisit(property:string) {
    const lead=createLead({name:customer.trim()||"Demo Customer",intent:`Site visit request · ${property}`,channel:"Demo",status:"New",prospectId:prospect?.id,businessName,location,budget:`Up to ₹${budget}L`});
  ...
  return <main style={{minHeight:"100vh",background:"#f7f8fa",color:"#17202a",fontFamily:"DM Sans,system-ui"}}>
    ...
    <section style={{maxWidth:1120,margin:"0 auto",padding:"52px 24px"}}>
      ...
      <div style={{background:"#101722",color:"#fff",borderRadius:20,padding:28,boxShadow:"0 20px 50px rgba(16,23,34,.15)"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,marginTop:28}}><div style={{background:"#fff",color:"#17202a",borderRadius:14,padding:18}}><label style={{fontSize:11,fontWeight:700}}>Budget up to ₹{budget}L</label><input type="range" min="40" max="100" value={budget} onChange={e=>setBudget(Number(e.target.value))} style={{width:"100%",margin:"16px 0"}}/><div style={{display:"grid",gap:9}}><div style={field}><Search size={14}/>{location}</div><div style={field}>2–3 BHK · Buy</div><label style={{fontSize:10,fontWeight:700}}>Your name</label><input value={customer} onChange={e=>setCustomer(e.target.value)} style={fieldInput}/><label style={{fontSize:10,fontWeight:700}}>Preferred site-visit time (optional)</label><input type="datetime-local" value={visitAt} onChange={e=>setVisitAt(e.target.value)} style={fieldInput}/><div style={{fontSize:10,color:"#697586"}}>Choose a property on the right to send a real BusinessOS lead + appointment request.</div></div></div><div style={{display:"grid",gap:10}}>{matches.map(p=><div key={p.name} style={{background:"#172130",border:"1px solid #2a3748",borderRadius:12,padding:15}}><div style={{display:"flex",justifyContent:"space-between",gap:10}}><strong style={{fontSize:13}}>{p.name}</strong><span style={{color:"#8eb0ff",fontSize:11}}>{p.tag}</span></div><div style={{marginTop:7,color:"#fff",fontWeight:800}}>{p.price}</div><button onClick={()=>requestVisit(p.name)} style={{marginTop:12,border:"1px solid #34445a",background:"transparent",color:"#fff",padding:"8px 10px",borderRadius:8,fontSize:11}}>Request site visit</button></div>)}</div></div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginTop:18}}>{[[MessageCircle,"WhatsApp enquiry","Customer can start a conversation from the property result."],[CalendarDays,"Site-visit request","The business receives a structured request with customer, property and preferred time."],[Phone,"Human handoff","A sales agent can take over when the lead is ready."]].map(([Icon,title,desc])=><div key={String(title)} style={{background:"#fff",border:"1px solid #e6e9ee",borderRadius:14,padding:18}}><div style={{width:34,height:34,borderRadius:9,background:"#eef3ff",display:"grid",placeItems:"center",color:"#2558e8"}}><Icon size={16}/></div><h3 style={{fontFamily:"Manrope",fontSize:14,margin:"12px 0 5px"}}>{String(title)}</h3><p style={{color:"#697586",fontSize:11,lineHeight:1.5}}>{String(desc)}</p></div>)}</div>
      {submitted&&<div style={{marginTop:18,padding:16,borderRadius:12,background:"#e9f8f1",color:"#11875d",fontSize:12}}><CheckCircle2 size={15} style={{verticalAlign:"-3px",marginRight:6}}/>Request captured in BusinessOS. Lead {leadId.slice(-8)} and a site-visit appointment request were created.</div>}
...
const fieldInput: React.CSSProperties={border:"1px solid #e6e9ee",borderRadius:8,padding:10,fontSize:11,width:"100%",boxSizing:"border-box"};
