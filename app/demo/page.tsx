...
export default function DemoFactoryPage(){
 ...
 return <main style={{minHeight:"100vh",background:"#f6f8fb"}}><header style={{height:64,background:"#fff",borderBottom:"1px solid #e6e9ee",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 24px",position:"sticky",top:0,zIndex:30}}><Link href={back} style={{color:"#697586",textDecoration:"none",fontSize:12}}><ArrowLeft size={14} style={{verticalAlign:"-2px",marginRight:5}}/>{from==="discovery"?"Back to Discovery":"Back to Sales"}</Link><span style={{fontSize:11,color:"#11875d",fontWeight:700}}><ShieldCheck size={13} style={{verticalAlign:"-2px",marginRight:5}}/>Demo preview · owner approval required</span></header>{resolvedProspect?<BlueprintWebsite prospect={resolvedProspect}/>:<div style={{padding:60,textAlign:"center",color:"#667085"}}>Prospect not found. Return to Discovery and create the demo again.</div>}</main>
}
