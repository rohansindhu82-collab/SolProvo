...
export async function auditWebsite(input: string): Promise<WebsiteAudit> {
  ...
  try {
    ...
    const missing = signals.filter((s) => ["leadCapture", "booking", "offers"].includes(s.key) && !s.detected);
    ...
    const positive = signals.filter((s) => s.detected).length;
    const score = Math.round((positive / signals.length) * 100);

    return {
      url: url.toString(), finalUrl, title, description, scannedAt: new Date().toISOString(), score,
      signals, opportunities,
      limitations: ["This audit scans the supplied public page only; it does not prove that a capability does not exist elsewhere on the business's website or offline.", "Signals are evidence-based detections, not claims about business performance.", "Before outreach, verify the business details and proposed gaps manually."],
    };
  } finally {
