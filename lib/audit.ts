...
function matches(html: string, pattern: RegExp) { return pattern.test(html); }
function evidence(html: string, pattern: RegExp, fallback: string) { const text = stripHtml(html); const match = text.match(pattern); return match?.[0] ? [match[0].slice(0, 120)] : [fallback]; }
function absoluteUrl(value: string, base: URL) { try { return new URL(value, base).toString(); } catch { return ""; } }
function extractImages(html: string, base: URL) {
  const values: string[] = [];
  const og = html.match(/<meta[^>]+property=["']og:image["'][^>]+content=["']([^"']+)["']/i)?.[1];
  if (og) values.push(absoluteUrl(og, base));
...
export function validatePublicUrl(input: string) {
  ...
  if (SOCIAL_HOSTS.has(host) || host.endsWith(".instagram.com") || host.endsWith(".facebook.com") || host.endsWith(".linkedin.com") || host.endsWith(".youtube.com")) throw new Error("Social media profiles are discovery references, not business websites, so they are not scored as website audits.");
  if (host === "localhost" || host === "127.0.0.1" || host === "0.0.0.0" || host === "::1" || host.endsWith(".local")) throw new Error("Private/local addresses cannot be analyzed.");
  if (/^(10\.|127\.|169\.254\.|192\.168\.)/.test(host) || /^172\.(1[6-9]|2\d|3[0-1])\./.test(host)) throw new Error("Private network addresses cannot be analyzed.");
  return url;
}
...
export async function auditWebsite(input: string): Promise<WebsiteAudit> {
  ...
  try {
    const response = await fetch(url, { signal: controller.signal, redirect: "follow", headers: { "User-Agent": "SolProvo-Website-Audit/0.1 (+https://solprovo.in)" }, cache: "no-store" });
    ...
    if (SOCIAL_HOSTS.has(finalHost) || finalHost.endsWith(".instagram.com") || finalHost.endsWith(".facebook.com") || finalHost.endsWith(".linkedin.com") || finalHost.endsWith(".youtube.com")) throw new Error("The URL redirected to a social media profile, not a business website.");
    ...
    const images = extractImages(html, finalBase);
    ...
    const opportunities = missing.map((s) => ({ key: s.key, title: s.key === "leadCapture" ? "Strengthen enquiry capture" : s.key === "booking" ? "Add a booking journey" : "Create a customer offer layer", reason: s.evidence[0] }));
    const score = Math.round((missing.length / opportunitySignals.length) * 100);
    return { url: url.toString(), finalUrl, title, description, images, scannedAt: new Date().toISOString(), score, signals, opportunities, limitations: ["This audit scans the supplied public page only; it does not prove that a capability does not exist elsewhere on the business's website or offline.", "Signals are evidence-based detections, not claims about business performance.", "Opportunity score measures only the verified absence of lead capture, booking and offer signals on the analyzed page.", "Before outreach, verify the business details and proposed gaps manually."] };
